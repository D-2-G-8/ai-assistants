...
const getNodeText = (node: Content | PhrasingContent): string => {
...
const getNodesText = (nodes: Content[]) =>
...
const cleanNode = (
  ...
  if ("children" in node && Array.isArray(node.children)) {
    const cleanedChildren = (node.children as Content[])
      .map((child) => cleanNode(child, options, stats))
      .flat()
      .filter(Boolean) as Content[];
    if (cleanedChildren.length === 0 && node.type !== "tableCell") {
      return null;
    }
    return { ...node, children: cleanedChildren };
  }
...
const cleanListItem = (
  ...
  const cleanedChildren = node.children
    .map((child) => cleanNode(child as Content, options, stats))
    .flat()
    .filter(Boolean) as Content[];
  if (cleanedChildren.length === 0) {
    stats.removedEmptyBlocks += 1;
...
const splitSections = (children: Content[]): Section[] => {
...
const rebuildChildren = (sections: Section[]): Content[] => {
...
export const buildOutline = (
  ...
  const visit = (nodes: Content[]) => {
    nodes.forEach((node) => {
      ...
      if ("children" in node && Array.isArray(node.children)) {
        visit(node.children as Content[]);
      }
    });
  };
