...
const createDecorations = (doc: Node, ranges: HighlightRange[]) => {
  const decorations = ranges.map((range) => {
    ...
    return Decoration.inline(safeRange.from, safeRange.to, {
      ...
      "data-finding-id": safeRange.findingId,
...
export const buildHighlightRanges = (doc: Node, findings: Finding[]) => {
  const { text, indexMap } = buildTextIndexMap(doc);
  const ranges: HighlightRange[] = [];
  const unmatched: string[] = [];

  const toHighlightSeverity = (
  ...
  findings
    ...
    .forEach((finding) => {
      const candidates = [finding.anchor?.quote, finding.anchor?.startHint]
      ...
      ranges.push({
        ...
        from,
        to: to + 1,
        severity: toHighlightSeverity(finding.severity),
  ...
  state: {
    ...
    apply(tr, prev) {
        ...
        map.forEach((from, to) => {
          prev.ranges.forEach((range) => {
            if (rangesIntersect(range.from, range.to, from, to)) {
              removedIds.add(range.findingId);
            }
          });
        });
      });
      ...
      const mappedRanges = prev.ranges
        .filter((range) => !removedIds.has(range.findingId))
        .map((range) => ({
          ...range,
